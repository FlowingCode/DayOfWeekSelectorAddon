...
//import { buttonStyles } from '@vaadin/button/vaadin-button-base.js';
import { ThemableMixin } from '@vaadin/vaadin-themable-mixin/vaadin-themable-mixin.js';
import { css, html, LitElement } from 'lit';
import { customElement, property, query, queryAssignedNodes, state } from 'lit/decorators.js';

@customElement('fc-day-of-week-selector')
export class DayOfWeekSelector extends ResizeMixin(ThemableMixin(LitElement)) {
    ...
    :host {
      ...
      --badge-list-badges-spacing: var(--lumo-space-m);
    }
    ...
    static get is() {
        return 'fc-day-of-week-selector';
    }
    ...
    __updateOnOverflow() {
        ...
        // get the badges list
        let daysOfWeek = this._daysOfWeek;
        ...
        if (removeFromIndex > -1) {
            for (let i = removeFromIndex; i < daysOfWeek.length; i++) {
                const element = daysOfWeek[i];
                element.setAttribute("hidden","");
            }
            // this.hiddenCount = daysOfWeek.length - removeFromIndex;
     ...
     */
    __updateHiddenItems() {
        const hiddenBadges = Array.from(this.querySelectorAll('[slot="daysOfWeek"][hidden]'));
        this.overflowItems = [];
        hiddenBadges.forEach(hiddenBadge => {
            const copy = hiddenBadge.cloneNode(true);
            copy.removeAttribute("slot");
            copy.removeAttribute("hidden");
            //TODO copied component doesn't have a click listener to update its state
            this.overflowItems.push({ component: copy });
        });
    }
    ...
    render() {
      ...
      <div part="container" class="container" id="container">
        ...
        <vaadin-context-menu open-on="click" .items=${this.overflowItems}>
        	<span part="overflow-badge" theme="${this.theme}" class="overflow-badge" hidden>
              <vaadin-icon icon="lumo:angle-right" style="padding: var(--lumo-space-xs)"></vaadin-icon>
            </span>
      	</vaadin-context-menu>        
      </div>
    `;
